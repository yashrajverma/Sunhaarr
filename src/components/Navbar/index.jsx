...
const Navbar = ({ cartItems, user, getSearch, search }) => {
  ...
  const navbarItems = useMemo(() => [
    { type: 'link', label: 'All Jewellery', href: '/products' },
  ...
  return (
    <div className="w-full bg-softPeach h-max py-3 border-b border-gray-600">
      <div className="block text-center mb-3 md:hidden">
        <Link to="/">
          <h1 className="animate-text bg-gradient-to-r from-accentGold to-yellow-700 bg-clip-text text-transparent font-bold text-3xl ">{BrandName}</h1>
        </Link>
      </div>
      <div className="flex justify-between px-6 md:px-16 items-center">
        ...
        <div className="hidden md:block">
          <Link to="/">
            <h1 className="animate-text bg-gradient-to-r from-accentGold to-yellow-700 bg-clip-text text-transparent text-3xl font-extrabold ">{BrandName}</h1>
          </Link>
        </div>

        {/* Center: Search */}
        <div className="flex items-center w-full md:w-[60%] border-2 border-primaryNavy border-opacity-75 px-2">
          <Icon iconName="magnifyingglass" size="w-4 h-4" />
          <div
            className="cursor-pointer rounded-md text-sm bg-transparent w-full border-none outline-none p-2 focus:border-none focus:outline-none focus:ring-0"
      ...
      {/* Mobile Sidebar */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-white bg-opacity-95 flex flex-col items-start pl-5 pt-5 h-fit">
